...
func (s *MySuite) TestToCheckTagsInSpecLevel(c *C) {
	...
	c.Assert(spec.Tags.Values[0][0], Equals, "tag1")
	c.Assert(spec.Tags.Values[0][1], Equals, "tag2")
}
...
func (s *MySuite) TestToCheckTagsInScenarioLevel(c *C) {
	...
	c.Assert(spec.Scenarios[0].Tags.Values[0][0], Equals, "tag1")
	c.Assert(spec.Scenarios[0].Tags.Values[0][1], Equals, "tag2")
}
...
func (s *MySuite) TestTableInputFromInvalidFileAndDataTableNotInitialized(c *C) {
	parser := new(SpecParser)
	specText := SpecBuilder().specHeading("Spec heading").text("table: inputinvalid.csv").text("comment").scenarioHeading("Sce heading").step("my step").String()
...
func (s *MySuite) TestTableInputFromFileIfPathNotSpecified(c *C) {
	parser := new(SpecParser)
	specText := SpecBuilder().specHeading("Spec heading").text("Table: ").scenarioHeading("Sce heading").step("my step").String()
...
func (s *MySuite) TestAddSpecTagsAndScenarioTags(c *C) {
	...
	c.Assert(len(tags.Values), Equals, 1)
	c.Assert(tags.Values[0][0], Equals, "tag3")
	c.Assert(tags.Values[0][1], Equals, "tag4")
}
...
* step
	...
	c.Assert(len(spec.Tags.Values), Equals, 1)
	c.Assert(spec.Tags.Values[0][0], Equals, "foo")
	c.Assert(spec.Tags.Values[0][1], Equals, "bar")
}
...
tags: blah
	...
	c.Assert(len(spec.Scenarios[0].Tags.Values), Equals, 1)
	c.Assert(spec.Scenarios[0].Tags.Values[0][0], Equals, "foo")
	c.Assert(spec.Scenarios[0].Tags.Values[0][1], Equals, "bar")
	c.Assert(len(spec.Tags.Values), Equals, 1)
	c.Assert(spec.Tags.Values[0][0], Equals, "tag1")
}
