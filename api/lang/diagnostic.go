...
func createDiagnosticsForDuplicateConcepts(pErr parser.ParseError, diagnostics []lsp.Diagnostic, uri string) []lsp.Diagnostic {
	values := strings.Split(pErr.Message, "\t")[1:]
	for _, val := range values {
		arr := strings.Split(val, ":")
		l, err := strconv.Atoi(strings.TrimSuffix(arr[len(arr)-1], "\n"))
		if err != nil {
			logger.Errorf("Error while getting line number for duplicate concepts: %s", err)
		}
		diagnostics = append(diagnostics, createDiagnostic(pErr.Message, l-1, 1, uri))
	}
	return diagnostics
}
