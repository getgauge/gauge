...
func GetSpecDir() string {
	specFromProperties := os.Getenv(env.SpecsDir)
	if specFromProperties != "" {
		// Return the first directory after trimming whitespace
		return strings.TrimSpace(strings.Split(specFromProperties, ",")[0])
	}
